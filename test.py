...
def bot_start(message):
    rmk = types.ReplyKeyboardMarkup(resize_keyboard=True, one_time_keyboard=True)  # Создание клавиатуры
    rmk.add(types.KeyboardButton("Да"), types.KeyboardButton("No"))  # Создание двух кнопок
    msg = client.send_message(message.chat.id, "Желаете подать заявку на модератора?", reply_markup=rmk)  # Добавление клавиатуры
